...
class CodebaseStore:
    """
    Workspace codebase-?? RAG store -- ChromaDB-?? code chunks embed ??? ?????
    Context-aware review ??? duplicate code detection-?? ???? ??????? ????
    """
    ...
    def _chunk_by_function_boundary(self, content: str, language: str) -> List[Dict[str, Any]]:
        """Code-?? function/class boundary ???????? chunk ????"""
        lines = content.split('\n')
        chunks = []
    ...
    def _fixed_size_chunks(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Function boundary ?? ????? fixed size-? chunk ????"""
        chunks = []
        i = 0
        while i < len(lines):
            end = min(i + self.CHUNK_MAX_LINES, len(lines))
    ...
    def index_file(self, file_path: str, content: str, language: Optional[str] = None) -> int:
        """???? file-?? content chunk ??? ChromaDB-?? upsert ???? Returns chunk count."""
    ...
    def get_context_for_diff(
        ...
        """Diff-?? ???? relevant codebase context string ???? ??? -- QualityAgent prompt-? ?????"""
        ...
        try:
            ...
            for match in similar:
                ...
                snippet = (
                    ...
                    f"(Line {match['start_line']}-{match['end_line']}, "
                    f"Similarity: {int(match['similarity']*100)}%)\n"
                    f"{match['text'][:500]}\n"
                )
